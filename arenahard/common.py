"""Common helpers for Arena-Hard v0.1."""
...
def ensure_question_file(config: Dict[str, Any]) -> Path:
    ...
    if configured_name != DEFAULT_QUESTION_FILE:
        raise FileNotFoundError(
            "Could not find Arena-Hard v0.1 question file. "
            "Set arenahard.question_file to a valid JSONL path."
        )
...
def load_questions(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        payload["category"] = str(row.get("category") or DEFAULT_BENCH_NAME)
        if row.get("cluster") is not None:
            payload["cluster"] = str(row["cluster"])
        normalized.append(payload)

    if not normalized:
        raise ValueError("Arena-Hard v0.1 question file is empty after normalization.")
...
def build_answer_row(
    *,
    ...
    return {
        ...
        "model": model_name,
        "messages": messages,
        "tstamp": float(question.get("tstamp", 0.0)) or time.time(),
