"""Inference pipeline for Arena-Hard v0.1."""
    ...
    *,
    ...
    if pending_questions:
        if mode == "local":
            ...
            raise ValueError("arenahard.mode must be 'local' or 'endpoint'.")
    ...
    print(f"[ArenaHard] model={get_pretty_name(config, BLOCK_NAME)}")
    print(f"[ArenaHard] mode={mode}")
    print(f"[ArenaHard] wrote_answer_file={answer_path}")
    return answer_path


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Arena-Hard v0.1 inference")
    parser.add_argument(
        "--config",
        type=str,
    ...
    run_arenahard_inference(config, endpoint_file=args.endpoint_file)
    return 0
