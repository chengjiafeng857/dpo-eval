"""Leaderboard reporting for Arena-Hard v0.1."""
...
def load_judgments(
    ...
    if invalid_rows:
        ...
        raise ValueError(
            "Invalid Arena-Hard v0.1 judgment rows found. "
...
def run_arenahard_report(
    ...
    *,
    ...
    resolved_judge_names = judge_names or [str(block_cfg.get("judge_model", DEFAULT_JUDGE_MODEL))]
    resolved_categories = categories or ["arena-hard-v0.1"]
    ...
    if not battles:
        raise FileNotFoundError("No Arena-Hard v0.1 judgment files were found.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Show Arena-Hard v0.1 results")
    parser.add_argument(
        "--config",
        type=str,
