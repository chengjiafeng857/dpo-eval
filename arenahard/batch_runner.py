"""Batch orchestration for Arena-Hard v2 inference and evaluation."""
...
def _build_model_config(
    ...
    *,
    ...
    if "model_endpoint" not in model_entry:
        raise ValueError("Each Arena-Hard batch model must define model_endpoint.")
    model_endpoint = model_entry["model_endpoint"]
    if not isinstance(model_endpoint, dict):
        raise ValueError("Arena-Hard batch model_endpoint must be a mapping.")

    pretty_name = str(
        model_entry.get("pretty_name")
        or model_endpoint.get("model")
        or model_endpoint.get("model_id")
        or "arenahard-model"
    )

    arenahard_cfg = config.setdefault("arenahard", {})
    arenahard_cfg["pretty_name"] = pretty_name
    arenahard_cfg["model_endpoint"] = copy.deepcopy(model_endpoint)
    arenahard_cfg["output_dir"] = str(Path("../outputs/arenahard") / sanitize_name(pretty_name))
        ...
        try:
            ...
            if do_evaluation:
                if skip_existing and _results_exist(config):
                    ...
                    run_arenahard_evaluation(config, model_answer_path=resolved_answers)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Arena-Hard v2 for a model batch")
    parser.add_argument(
        "--config",
        type=str,
